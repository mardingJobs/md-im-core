 ...
 */
package cn.md.im.core.client.sender.impl;

import cn.hutool.core.collection.CollectionUtil;
import cn.hutool.core.util.StrUtil;
import cn.md.im.core.client.sender.IMSender;
import cn.md.im.core.common.cache.DistributedCacheService;
import cn.md.im.core.common.constants.IMConstants;
import cn.md.im.core.common.enums.IMListenerType;
import cn.md.im.core.common.enums.IMSendCode;
import cn.md.im.core.common.enums.IMTerminalType;
import cn.md.im.core.common.model.*;
import cn.md.im.core.common.mq.MessageSenderService;
...
public class DefaultIMSender implements IMSender {
        ...
        for (Integer terminal : IMTerminalType.codes()){
            ...
            if (!terminal.equals(message.getSender().getTerminal())){
                ...
                if (!StrUtil.isEmpty(serverId)){
                    IMReceiveInfo imReceiveInfo = new IMReceiveInfo(IMMsgType.GROUP_MESSAGE.code(), message.getSender(), Collections.singletonList(new IMUserInfo(message.getSender().getUserId(), terminal)), false,message.getData());
    ...
    private <T> void sendGroupMessageToOtherUsers(Map<Integer, List<IMUserInfo>> serverMap, List<IMUserInfo> offlineUserList, IMGroupMessage<T> message) {
        for (Map.Entry<Integer, List<IMUserInfo>> entry : serverMap.entrySet()){
            IMReceiveInfo imReceiveInfo = new IMReceiveInfo(IMMsgType.GROUP_MESSAGE.code(), message.getSender(), new LinkedList<>(entry.getValue()), message.getSendResult(), message.getData());
